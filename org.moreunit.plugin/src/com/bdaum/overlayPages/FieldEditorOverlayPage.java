...
public abstract class FieldEditorOverlayPage
	...
	public void createControl(final Composite parent) {
		...
		if (isPropertyPage()) {
			...
			this.overlayStore =
				new PropertyStore(
					((IJavaProject)getElement()).getProject(),
	...
	private void createSelectionGroup(final Composite parent) {
		...
		this.useWorkspaceSettingsButton = createRadioButton(radioGroup, Messages.getString("OverlayPage.Use_Workspace_Settings")); //$NON-NLS-1$
		this.useProjectSettingsButton = createRadioButton(radioGroup, Messages.getString("OverlayPage.Use_Project_Settings")); //$NON-NLS-1$
		this.configureButton = new Button(comp, SWT.PUSH);
		this.configureButton.setText(Messages.getString("OverlayPage.Configure_Workspace_Settings")); //$NON-NLS-1$
		...
		try {
			String use =
				((IJavaProject) getElement()).getProject().getPersistentProperty(
					new QualifiedName(this.pageId, FieldEditorOverlayPage.USEPROJECTSETTINGS));
			if (FieldEditorOverlayPage.TRUE.equals(use)) {
	...
	public boolean performOk() {
		...
		if (result && isPropertyPage()) {
			...
			try {
				String value =
					(this.useProjectSettingsButton.getSelection()) ? FieldEditorOverlayPage.TRUE : FieldEditorOverlayPage.FALSE;
